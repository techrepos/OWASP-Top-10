using ABCBankingWebAppAdmin.Extensions;
...
namespace ABCBankingWebAppAdmin.Services
{
    public class KnowledgebaseService : IKnowledgebaseService
    {
        ...
        public List<Knowledge> Search(string input)
        {
            ...
            var file = Path.Combine(webRoot, "Knowledgebase.xml");
            ...
            foreach (XPathNavigator node in matchedNodes)
            {
                ...
                {
                    Topic = node.SelectSingleNode(nav.Compile("topic")).Value,
                    Description = node.SelectSingleNode(nav.Compile("description")).Value
                });
            }
